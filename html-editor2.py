...
def save_html_file():
    if soup:
        ...
        if reg_number_tag and reg_number_tag.find_next('br'):
            reg_number_tag.find_next('br').replace_with(f"\n{reg_number_var.get()}")

        audit_ref_tag = soup.find('h3', text=lambda t: t and 'Audit Reference Number' in t)
        if audit_ref_tag and audit_ref_tag.find_next('br'):
            audit_ref_tag.find_next('br').replace_with(f"\n{audit_ref_var.get()}")

        cert_number_tag = soup.find('h3', text=lambda t: t and 'Certified Number' in t)
        if cert_number_tag and cert_number_tag.find_next('br'):
            cert_number_tag.find_next('br').replace_with(f"\n{cert_number_var.get()}")

        eff_date_tag = soup.find('h3', text=lambda t: t and 'Effective Date Until' in t)
        if eff_date_tag and eff_date_tag.find_next('br'):
            eff_date_tag.find_next('br').replace_with(f"\n{eff_date_var.get()}")

        scope_tag = soup.find('h3', text=lambda t: t and 'Scope Of Certification' in t)
        if scope_tag and scope_tag.find_next('br'):
            scope_tag.find_next('br').replace_with(f"\n{scope_var.get()}")
