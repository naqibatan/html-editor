...
def save_html_file():
    if soup:
        ...
        if reg_number_tag and reg_number_tag.find_next_sibling():
            reg_number_tag.find_next_sibling().replace_with(f"{reg_number_var.get()}")

        # Update Audit Reference Number
        audit_ref_tag = soup.find('h3', text=lambda t: t and 'Audit Reference Number' in t)
        if audit_ref_tag and audit_ref_tag.find_next_sibling():
            audit_ref_tag.find_next_sibling().replace_with(f"{audit_ref_var.get()}")

        # Update Certified Number
        cert_number_tag = soup.find('h3', text=lambda t: t and 'Certified Number' in t)
        if cert_number_tag and cert_number_tag.find_next_sibling():
            cert_number_tag.find_next_sibling().replace_with(f"{cert_number_var.get()}")

        # Update Effective Date Until
        eff_date_tag = soup.find('h3', text=lambda t: t and 'Effective Date Until' in t)
        if eff_date_tag and eff_date_tag.find_next_sibling():
            eff_date_tag.find_next_sibling().replace_with(f"{eff_date_var.get()}")

        # Update Scope Of Certification
        scope_tag = soup.find('h3', text=lambda t: t and 'Scope Of Certification' in t)
        if scope_tag and scope_tag.find_next_sibling():
            scope_tag.find_next_sibling().replace_with(f"{scope_var.get()}")
